...
(function ($) {
	$(document).ready(function () {
		$('#woocommerce_pay360_hosted_cashier_skin_default').on('change', pay360Admin.maybeHideSkin).change();
		$('#woocommerce_pay360_hosted_cashier_use_iframe').on('change', pay360Admin.maybeHideIframeFields).change();
		$('#woocommerce_pay360_cashier_use_3ds').on('change', pay360Admin.maybeHideThreeDS).change();
	...
	var pay360Admin = {
		...
		maybeHideThreeDS: function () {
			var field = $('#woocommerce_pay360_cashier_use_3ds');
			var addField1 = $('#woocommerce_pay360_cashier_challenge_requested');

			if (field.is(':checked')) {
				pay360Admin.showField(addField1);
				return;
			}

			pay360Admin.hideField(addField1);
		},
