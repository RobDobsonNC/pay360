<?php
/*
 * Plugin Name: WooCommerce Pay360 Gateway
 * Plugin URI: https://woocommerce.com/products/pay360/
 * Description: Allows you to use <a href="https://www.pay360.com/">Pay360</a> payment gateway with the WooCommerce plugin.
 * Version: 2.4.0
 * Author: VanboDevelops
 * Author URI: https://www.vanbodevelops.com
 ...
 *
 *	Copyright: (c) 2012 - 2023 VanboDevelops
...
class WC_Pay360 {
	...
	public function update_procedure() {
		$updates = new \WcPay360\Updates\Updates_Manager();
		$updates->load();
		$updates->hooks();
	}
	...
	public function settings_support_link( $actions, $plugin_file, $plugin_data, $context ) {
		...
		return array_merge(
			array(
				...
				'support'  => '<a href="https://woocommerce.com/my-account/contact-support/">' . __( 'Support', self::TEXT_DOMAIN ) . '</a>',
			),
			$actions
		);
	}
	...
	public function is_hpos_compatible() {
		if ( apply_filters( 'wc_pay360_should_declare_hpos_compatibility', true, __FILE__ )
