...
class WC_Pay360_Gateway extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->renewals_method                = $this->get_option( 'hosted_cashier_renewals_method', 'repeat' );
	...
	public function get_integration_options() {
		return array(
			...
			'hosted'         => __( 'Pay360 Hosted', WC_Pay360::TEXT_DOMAIN ),
			'hosted_ima'     => __( 'Pay360 Hosted + IMA', WC_Pay360::TEXT_DOMAIN ),
		);
	}
	...
	public function receipt_page( $order_id ) {
		try {
			echo apply_filters( 'wc_pay360_before_payment_form_html', '<p>' . esc_html( __( 'Thank you for your order, please click the button below to pay.', WC_Pay360::TEXT_DOMAIN ) ) . '</p>', $this->integration, $this );
			...
			wc_print_notice( \WcPay360\Helpers\Formatting::kses_form_html( $e->getMessage() . ' ' . __( 'Please reload the page or contact administrator.', WC_Pay360::TEXT_DOMAIN ) ), 'error' );
		}
	}
	...
	public function generate_text_html( $key, $data ) {
		...
		if ( $is_password_field ) {
			$data['placeholder'] = sprintf( __( '%s is not set', WC_Pay360::TEXT_DOMAIN ), strip_tags( $data['title'] ) );
			if ( '' != $value ) {
				$data['placeholder'] = sprintf( __( '%s is set', WC_Pay360::TEXT_DOMAIN ), strip_tags( $data['title'] ) );
			}
			$value = '';
		}
