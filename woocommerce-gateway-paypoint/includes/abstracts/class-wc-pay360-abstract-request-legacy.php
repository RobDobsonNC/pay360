...
/**
 * Abstract Request class to serve as a basis for legacy integration requests.
 *
 * @since  1.2
...
abstract class WC_Pay360_Abstract_Request_Legacy extends WC_Pay360_Abstract_Request {
	...
	public function get_form_output( $order, $url, $form_fields ) {
		return '<form action="' . esc_url( $url ) . '" method="post" id="pay360_payment_form" target="_top">
			' . $form_fields . '
			<input type="submit" class="button-alt" id="submit_pay360_payment_form" value="' . esc_attr( __( 'Pay via Pay360', WC_Pay360::TEXT_DOMAIN ) ) . '" /> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">' . esc_html( __( 'Cancel order &amp; restore cart', WC_Pay360::TEXT_DOMAIN ) ) . '</a>
		    </form>';
	}
}