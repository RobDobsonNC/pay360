...
abstract class WC_Pay360_Abstract_Integration {
	 ...
	 *
	 * @since      2.0
	 * @deprecated 2.3.0: Not well worded so we renamed it
	 *
	 * @param \WC_Order $order Optional
	 *
	 * @return bool
	 */
	public function maybe_do_3d_secure( $order = false ) {
		return $this->is_3ds_enabled( $order );
	}
	 ...
	 * @return bool
	 */
	public function is_3ds_enabled( $order = false ) {
		$is_3ds = true;
		if ( 'no' == $this->get_gateway()->get_option( 'cashier_use_3ds' ) ) {
			$is_3ds = false;
		}
		
		return apply_filters( 'wc_pay360_hosted_cashier_do_3d_secure', $is_3ds, $order );
	}
	...
	public function get_merchant_customer_id( $user_id ) {
		...
		if ( ! $merchant_id ) {
			...
			$merchant_id = str_replace( array( ' ' ), array( '-' ), $this->get_gateway()->get_option( 'cashier_customer_prefix' ) ) . $user_id;
	...
	public function get_guest_merchant_customer_id() {
		return str_replace( array( ' ' ), array( '-' ), $this->get_gateway()->get_option( 'cashier_customer_prefix' ) ) . uniqid() . $this->get_guest_customers_suffix();
	}
