...
class WC_Pay360_Hosted_Cashier_Response extends WC_Pay360_Abstract_Response {
	...
	public function route_response() {
		...
		try {
			...
			$ref_number = wc_clean( WC_Pay360::get_field( 'merchantRef', $_GET, '0:0:0' ) );
			...
			if ( $pay360_order->is_subscription() && $is_change_payment_method ) {
	 ...
	 * @param                            $response
	 */
	public function process_payment_method_change_response( $subscription, $response ) {
		if ( 'SUCCESS' == $response->transaction->status ) {
			do_action( 'pay360_hosted_cashier_successful_change_payment_method', $subscription, $response->transaction );
			
			$this->save_meta_data_to_subscription( $subscription, $response );
			...
			do_action( 'pay360_hosted_cashier_failed_change_payment_method', $subscription, $fail_message, $resp_code );
		}
	}
	...
	public function get_order_from_merchant_reference( $reference ) {
		...
		if ( ! $order ) {
			wp_die( esc_html( __( 'Pay360 Response Failure. Invalid order id.', WC_Pay360::TEXT_DOMAIN ) ), 'Pay360 Response', array( 'response' => 200 ) );
		}
	...
	public function process_response( \WC_Order $order, $response ) {
		if ( 'SUCCESS' == $response->transaction->status ) {
			...
			if ( $this->is_deferred() ) {
				...
				do_action( 'pay360_hosted_cashier_successful_deferred_transaction', $order, $response->transaction, $response );
			} else {
				...
				do_action( 'pay360_hosted_cashier_successful_transaction', $order, $response->transaction, $response );
			}
	...
	public function hash_parameters() {
		$hash_string = ! empty( $_SERVER['REQUEST_URI'] ) ? wc_clean( $_SERVER['REQUEST_URI'] ) : '';
	...
	protected function end_execution( $order = false ) {
		...
		if ( false === $order ) {
			$redirect_to = apply_filters( 'wc_pay360_no_order_response_url', wc_get_cart_url(), $this->get_gateway() );
			if ( $this->use_iframe() ) {
				wc_get_template(
		...
		if ( $pay360_order->is_subscription() ) {
			// Return the customer to the subscription page and not to the Thank you, if we are just changing the payment method
			$redirect_url = $order->get_view_order_url();
		}
		
		$redirect_url = apply_filters( 'wc_pay360_response_return_url', $redirect_url, $this->get_gateway(), $order );
		if ( $this->use_iframe() ) {
			wc_get_template(
				'pay360/iframe-break.php',
				array(
