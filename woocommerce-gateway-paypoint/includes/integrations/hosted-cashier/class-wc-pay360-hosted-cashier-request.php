...
class WC_Pay360_Hosted_Cashier_Request extends WC_Pay360_Abstract_Request {
	...
	public function get_payment_parameters( WC_Order $order ) {
		...
		$args = array(
			...
			'order' => array(
				'orderRef' => 'order_' . $order->get_id(),
			),
		);
		
		$threeds = new \WcPay360\Integrations\Hosted_Cashier\Three_DS2( $order, $this );
		$args    = $threeds->add_strong_authentication_params( $args );
		...
		if ( $pay360_order->contains_subscription() ) {
			$args['transaction']['recurring'] = true;
		}
	...
	public function get_verification_parameters( WC_Order $order ) {
		...
		$pay360_order = new \WcPay360\Pay360_Order( $order );
		if ( $pay360_order->contains_subscription() || $is_change_payment_method ) {
			$args['transaction']['recurring'] = true;
		}
