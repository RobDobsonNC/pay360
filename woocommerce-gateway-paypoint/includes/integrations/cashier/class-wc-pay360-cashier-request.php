...
class WC_Pay360_Cashier_Request extends WC_Pay360_Abstract_Request {
	...
	public function merchant_token_payment( $order, $amount = null, $token = '', $customer_id = '' ) {
		...
		$arguments = apply_filters( 'wc_pay360_merchant_token_payment_params', $arguments, $order, $amount, $token, $customer_id );
	 ...
	 */
	public function repeat_payment( $order, $amount = 0, $transaction_id = '' ) {
		if ( ! $order instanceof \WC_Order && is_numeric( $order ) ) {
			$order = wc_get_order( (int) $order );
		}
		...
		WC_Pay360_Debug::add_debug_log( 'Start Merchant Repeat Payment' );
		
		$pay360_order = new \WcPay360\Pay360_Order( $order );
		
		if ( '' === $transaction_id ) {
			$transaction_id = $pay360_order->get_initial_transaction_id();
			// If still no transaction ID
			if ( ! $transaction_id ) {
				$updates        = new \WcPay360\Updates\Subscriptions_Update();
				$subscription   = wcs_get_subscriptions_for_order( $order, ['order_type'=>'any'] );
				$transaction_id = $updates->attempt_to_populate_initial_transaction( array_shift( $subscription ) );
			}
		}
		
		// At this point we tried to get a transaction and are still not successful, let's just fail it then
		if ( empty( $transaction_id ) ) {
			throw new \WcPay360\Api\Exceptions\Exception( __( 'The transaction ID is not set or empty.', WC_Pay360::TEXT_DOMAIN ) );
		}
		
		$api = $this->get_api_client();
		
		WC_Pay360_Debug::add_debug_log( 'Repeat Request for transaction ID: ' . print_r( $transaction_id, true ) );
		
		$desc               = sprintf( __( '[%s] Repeat Payment for order %s', WC_Pay360::TEXT_DOMAIN ), wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $order->get_order_number() );
		$suffix             = $this->get_attempts_suffix( $order );
		$merchant_reference = WC_Pay360_Compat::get_prop( $order, 'id' ) . ':' . WC_Pay360::get_order_number( $order ) . ':' . WC_Pay360_Compat::get_prop( $order, 'order_key' ) . ':' . $suffix;
		...
		if ( 0 < $amount ) {
			$arguments['transaction']['amount']   = $amount;
			$arguments['transaction']['currency'] = $this->get_currency( $order );
		}
		
		$arguments = apply_filters( 'wc_pay360_repeat_payment_arguments', $arguments, $order, $amount, $transaction_id );
		
		WC_Pay360_Debug::add_debug_log( 'Repeat Request $arguments: ' . print_r( $arguments, true ) );
		
		$result = $api->get_cashier_service()->process_repeat_payment( $transaction_id, $arguments );
		
		WC_Pay360_Debug::add_debug_log( 'Repeat Payment Response: ' . print_r( $result, true ) );
