...
class WC_Pay360_Cashier_Response extends WC_Pay360_Abstract_Response {
	...
	public function get_merchant_reference_from_response( $response ) {
		...
		} elseif ( isset( $response->transaction ) && isset( $response->transaction->relatedTransaction ) ) {
			return $response->transaction->relatedTransaction->merchantRef;
		}
	...
	public function process_repeat_payment_response( WC_Order $order, $response ) {
		//Debug log
		WC_Pay360_Debug::add_debug_log( __( 'Process Cashier API repeat response.', WC_Pay360::TEXT_DOMAIN ) );
		
		if ( 'SUCCESS' == $response->transaction->status ) {
			if ( $response->transaction->deferred ) {
				// The money were not taken, so we don't want to Process the order just, yet.
				$order->update_status( apply_filters( 'wc_pay360_deferred_payment_order_status', 'on-hold', $order, $this ), __( 'Transaction was successfully Authorized.', WC_Pay360::TEXT_DOMAIN ) );
			...
			//Debug log
			WC_Pay360_Debug::add_debug_log( __( 'Repeat cashier payment completed.', WC_Pay360::TEXT_DOMAIN ) );
		} else {
	...
	public function validate_repeat_response( WC_Order $order, $response = null ) {
		// Order should not be paid for
		$this->validate_order_not_paid( $order );
