...
class WC_Pay360_Gateway_Addons extends WC_Pay360_Gateway {
	...
	public function scheduled_subscription_payment_request( $amount_to_charge, $renewal_order ) {
		try {
			...
			if ( 'token' == $this->renewals_method ) {
				...
				$response = $integration->repeat_payment( $renewal_order, $amount_to_charge );
				$api_response->validate_repeat_response( $renewal_order, $response );
				$api_response->process_repeat_payment_response( $renewal_order, $response );
			}
		}
		catch ( Exception $e ) {
	...
	public function changed_failing_payment_method( $subscription, $renewal_order ) {
		...
		if ( 'token' === $this->renewals_method ) {
			...
			$pay360_sub->save_initial_transaction_id( $pay360_renewal->get_initial_transaction_id() );
		}
	...
	public function add_subscription_payment_meta( $payment_meta, $subscription ) {
		// New subscription object because the passed had old data
		$subscription_new = wcs_get_subscription( $subscription->get_id() );
		$pay360_order     = new \WcPay360\Pay360_Order( $subscription_new );
		...
		if ( 'token' === $this->renewals_method ) {
			...
			$payment_meta[ $this->id ]['post_meta']['_wc_pay360_initial_transaction_id'] = array(
				'value' => $pay360_order->get_initial_transaction_id(),
				'label' => 'Pay360 Initial Transaction ID',
			);
		}
	...
	public function validate_subscription_payment_meta( $payment_method_id, $payment_meta ) {
		...
		if ( 'token' === $this->renewals_method ) {
			...
			if ( ! isset( $payment_meta['post_meta']['_wc_pay360_initial_transaction_id']['value'] )
			     || empty( $payment_meta['post_meta']['_wc_pay360_initial_transaction_id']['value'] )
			) {
				throw new Exception( 'A Pay360 Initial Transaction ID is required.' );
			}
		}
	}
