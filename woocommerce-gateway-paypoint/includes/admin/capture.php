...
class Capture {
	...
	public function order_meta_box_add_capture_payment_buttons( $order ) {
		...
		<button type="button" class="button button-primary wc-pay360-capture-payment-init">
			<?php echo Formatting::kses_form_html( sprintf( __( 'Capture Transaction (%s)', \WC_Pay360::TEXT_DOMAIN ), get_woocommerce_currency_symbol( \WC_Pay360_Compat::get_order_currency( $order ) ) . $authorized_amount ) ); ?>
		</button>
		<?php
	}
